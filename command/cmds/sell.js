...
exports.describe = 'Sell coin - Manual Mode'
...
exports.handler = exchangeRequire((user, exchangeName, exchangeClient, argv) => {
  ...
  Exchange.sellLimit(exchangeClient)(market, quantity, rate)
  .then(orderId => {
    return DB.createOrder(exchangeName, orderId, user, 'manual', 'short', market, rate, btc, quantity)
    .catch(error => {
      // Rollback order
